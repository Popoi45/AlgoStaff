...
class Binance_API:
    ...
    futures_link = 'https://testnet.binancefuture.com'
    # testeneet_link = 'https://testnet.binance.vision'
    ...
    def post_market_order(self, symbol: str, side, qnt: float = None, quoteOrderQty: float = None):
        if not qnt and not quoteOrderQty:
            raise Exception("Не указан один из обизательных параметорв quantity или quoteOrderQty")
        if self.futures:
