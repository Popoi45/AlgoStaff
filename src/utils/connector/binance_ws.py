...
class BinanceWs(BaseWs):
    def __init__(self, shutdown_event, topics=None, handler=None):
        ...
        self.url = "wss://stream.testnet.binance.vision:9443/ws" if TEST_MODE else "wss://stream.binance.com:9443/ws"
    ...
    async def subscribe_to_topics(self, topics):
        ...
        if topics and self.is_connected:
            data = {"method": "SUBSCRIBE", "params": topics, "id": 55151}
            await self.ws.send(json.dumps(data))

    async def unsubscribe_from_topics(self, topics):
        if topics and self.is_connected:
            data = {"method": "UNSUBSCRIBE", "params": topics, "id": 55152}
            await self.ws.send(json.dumps(data))
