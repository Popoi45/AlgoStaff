...
from .endpoints.delivery import Endpoints
from .symbols import SymbolDelivery


class Delivery(Client, Endpoints):
    _symbol_class = SymbolDelivery

    def __init__(self, api_key=None, secret_key=None, testnet=False, asynced=False):
        self.base_url = 'https://testnet.binancefuture.com' if testnet else 'https://dapi.binance.com'
        super().__init__(api_key, secret_key, testnet=testnet, asynced=asynced)

    def load_symbols(self) -> dict[str, SymbolDelivery]:
    ...
    def websocket(self, stream=None, on_message=None, on_open=None, on_close=None, on_error=None):
        if self.asynced:
            ...
            return DeliveryWebsocketSync(stream=stream, on_message=on_message, on_open=on_open, on_close=on_close,
    ...
    async def _websocket_async(self, stream=None, on_message=None, on_open=None, on_close=None, on_error=None):
        ws = DeliveryWebsocketAsync(stream=stream, on_message=on_message, on_open=on_open, on_close=on_close,
                                   on_error=on_error, testnet=self.testnet)
        await ws.run()
        return ws

    def websocket_userdata(self, on_message=None, on_open=None, on_close=None, on_error=None):
        listen_key_url = f'{self.base_url}/dapi/v1/listenKey'
        if self.asynced:
            ...
            return DeliveryWebsocketSync(on_message=on_message, on_open=on_open, on_close=on_close,
                                        on_error=on_error, testnet=self.testnet, api_key=self.api_key,
                                        listen_key_url=listen_key_url)

    async def _websocket_userdata_async(self, stream=None, on_message=None, on_open=None, on_close=None,
                                        on_error=None, listen_key_url=None):
        ws = DeliveryWebsocketAsync(stream=stream, on_message=on_message, on_open=on_open, on_close=on_close,
                                   on_error=on_error, testnet=self.testnet, api_key=self.api_key,
                                   listen_key_url=listen_key_url)
        await ws.run()
...
class DeliveryWebsocketSync(WebsocketSync):
    def __init__(self, stream=None, on_message=None, on_open=None, on_close=None, on_error=None, testnet=False,
        ...
        self.base_url = 'wss://dstream.binancefuture.com' if testnet else 'wss://dstream.binance.com'
        super().__init__(stream, on_message, on_open, on_close, on_error, testnet, api_key, listen_key_url)


class DeliveryWebsocketAsync(WebsocketAsync):
    def __init__(self, stream=None, on_message=None, on_open=None, on_close=None, on_error=None, testnet=False,
                 api_key=None, listen_key_url=None):
        self.base_url = 'wss://dstream.binancefuture.com' if testnet else 'wss://dstream.binance.com'
        super().__init__(stream, on_message, on_open, on_close, on_error, testnet, api_key, listen_key_url)